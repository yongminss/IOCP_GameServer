...
void ProcessPacket(int ci, unsigned char packet[])
{
	...
	case S2C_MOVE: {
		...
		if (move_packet->id < MAX_CLIENTS) {
			int my_id = client_map[move_packet->id]; // 서버에서 쓰는 ID가 테스트 클라이언트에서 ID는 이것이다를 명시
			// 테스트 클라이언트에서 접속하지 않은 플레이어의 위치는 받지 않음
			if (-1 != my_id) {
				// 테스트 클라이언트에서 사용하는 id와 게임 서버 ID 다름
				g_clients[my_id].x = move_packet->x;
				g_clients[my_id].y = move_packet->y;
			}
			// 소켓을 받은 ID와 현재 ID가 같다면? - 내가 내 데이터를 받음
			if (ci == my_id) {
				if (0 != move_packet->move_time) {
					...
					// 숫자가 튈 수 있기 때문에 조금씩 반영해서 실제 딜레이에 맞춤
				   ...
				   break;
	case S2C_ENTER: break; // 다른 플레이어의 login은 처리할 필요 없음
...
void Test_Thread()
{
	while (true) {
		...
		Adjust_Number_Of_Client(); // 동접을 차근차근 증가시키기 위해 사용
