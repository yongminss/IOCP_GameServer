...
#include "..\..\GameServer\IOCP_Server\protocol.h"
...
void ProcessPacket(char* ptr)
{
	...
	case S2C_LOGIN_OK:
	{
		...
		// 아바타가 화면의 가운데에서 보이도록 그릴 맵의 x, y 좌표
	...
	case S2C_RECALL:
	{
		is_player = true;
		avatar.show();
		char text[16] = "comeback!";
		avatar.add_chat(text);
	}
	break;
...
void client_main()
{
	...
	for (int i = 0; i < SCREEN_WIDTH; ++i)
		for (int j = 0; j < SCREEN_HEIGHT; ++j)
		{
			...
			//if (((tile_x + tile_y) % 2) == 0) { // 1칸 마다 색깔이 바뀜
			if (((tile_x / 3 + tile_y / 3) % 2) == 0) { // 3칸마다 색깔이 바뀜
	...
	char buf[100];
	sprintf_s(buf, "HP - %d Level - %d Exp - (%d / %d) Pos - (%d, %d)", avatar.m_hp, avatar.m_level, avatar.m_exp, avatar.m_level * 50, avatar.m_x, avatar.m_y);
	text.setString(buf);
	g_window->draw(text);
...
void send_chat_packet(char message[])
{
	cs_packet_chat m_packet;
	m_packet.size = sizeof(m_packet);
	m_packet.type = C2S_CHAT;
	strcpy_s(m_packet.mess, message);
	send_packet(&m_packet);
}
...
int main()
{
	...
	l_packet.id = input_id;
	int t_id = GetCurrentProcessId();
	sprintf_s(l_packet.name, "P%03d", input_id);
	...
	bool is_chatting = false; // Chatting을 활성화 하는 bool
	bool send_chatting = false; // chatting을 보내는 걸 bool
	...
	while (window.isOpen())
	{
		...
		while (window.pollEvent(event))
		{
			...
			// chatting
			if (send_chatting) {
				send_chat_packet(mess);
				send_chatting = false;
				for (int i = 0; i < MAX_STR_LEN; ++i) mess[i] = 0;
			}
